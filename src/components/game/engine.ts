...
import { EL_LIST, VILLAGE_COLORS, type ElementId, type VillageId, type FxData, type DmgData, type SnapshotData, type WelcomeData, type YouState, maxHpOf, TILE } from './types';
export interface Entity {
    id: number; kind: 'player' | 'monster'; n: string; lv: number; el?: ElementId; village?: VillageId; pal: number; t?: string;
...
export interface HudState {
    ...
    interact: 'fonte' | 'loja' | 'equipshop' | 'bounty' | null;
    fps: number;
    selfMoving: boolean;
}
const OBJ_FOOTPRINT = {
...
export class GameEngine {
    ...
    constructor(canvas: HTMLCanvasElement, welcome: WelcomeData, input: InputController, art: GameArt) {
        ...
        this.village = welcome.self.village;
        ...
        const initialHp = maxHpOf(welcome.self.lv) + (welcome.self.equipmentStats?.hp || 0);
    ...
    restoreObject(d: { id: number; k: string; x: number; y: number; hp?: number }) {
        if (this.map.objects.some((o) => o.id === d.id)) return;
        const obj = { id: d.id, k: d.k, x: d.x, y: d.y, ...(d.hp != null ? { hp: d.hp } : {}) };
        const [fw, fh] = OBJ_FOOTPRINT[obj.k] || [1, 1];
        this.map.objects.push(obj);
        this.objRender.push({ id: obj.id, k: obj.k, x: obj.x, y: obj.y, sortY: (obj.y + fh) * TILE });
        for (let oy = 0; oy < fh; oy++) {
            for (let ox = 0; ox < fw; ox++) {
                const tx = obj.x + ox, ty = obj.y + oy;
                if (tx < 0 || ty < 0 || tx >= this.map.w || ty >= this.map.h) continue;
                this.walkGrid[ty * this.map.w + tx] = 1;
            }
        }
        this.minimapBase = null;
        const cx = (obj.x + fw / 2) * TILE, cy = (obj.y + fh / 2) * TILE;
        const rock = obj.k === 'rock';
        const colors = rock ? ['#c4b8a9','#988a7c','#74675c'] : ['#9fd06a','#6a9a47','#4d7135'];
        for (let i = 0; i < (rock ? 9 : 14); i++) {
            const a = Math.random() * Math.PI * 2, speed = 18 + Math.random() * 40;
            this.particles.push({
                x: cx + (Math.random() - .5) * 12,
                y: cy + 8 + (Math.random() - .5) * 8,
                vx: Math.cos(a) * speed,
        ...
        const touch = (id, x, y, dir, t) => {
            ...
            if (!e) {
                ...
                if (nf?.k === 'm') {
                    ...
                    const village = player.v || 'folha';
                    const pal = player.pal ?? 0;
                    e = { id, kind:'player', n:player.n||'???', lv, el, village, pal, hp:maxHpOf(lv), hpPct:100, dir:0, ax:x, ay:y, at:t-50, bx:x, by:y, bt:t, x,y,moving:false,animT:0,lastSnapLocal:localT,flashUntil:0,lastHp:maxHpOf(lv) };
                }
                ...
                if(nf){
                    e.lv=nf.lv;
                    if(nf.k==='p'){
                        e.n=nf.n||e.n;e.el=nf.el||e.el;e.village=nf.v||e.village;e.pal=nf.pal??e.pal;
                        this.rosterMeta.set(id,{id,n:e.n,lv:nf.lv,el:e.el||'fogo',v:e.village||'folha',pal:e.pal});
    ...
    getAim(){if(this.input.hasRecentMouse()){const rect=this.canvas.getBoundingClientRect();return{x:this.camX+(this.input.state.aimX-rect.width/2)/this.zoom,y:this.camY+(this.input.state.aimY-rect.height/2)/this.zoom};}const t=this.findTarget(620);if(t)return{x:t.x,y:t.y-6};const d=[[0,1],[-.707,.707],[-1,0],[-.707,-.707],[0,-1],[.707,-.707],[1,0],[.707,.707]][this.selfDir]||[0,1];return{x:this.selfX+d[0]*190,y:this.selfY+d[1]*190};}
    findTarget(range: number): Entity | null {let best: Entity | null=null,bestScore=Infinity;for(const e of this.entities.values()){if(e.id===this.selfId||e.hpPct<=0)continue;if(e.kind==='player'&&(e.village===this.village||this.zoneSafe||this.zoneOf(e.x,e.y).safe))continue;const d=Math.hypot(e.x-this.selfX,e.y-this.selfY);if(d>=range)continue;const score=d+(e.kind==='player'?90:0);if(score<bestScore){best=e;bestScore=score;}}return best;}
