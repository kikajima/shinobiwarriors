...
export class GameEngine {
    ...
    findTarget(range: number): Entity | null {let best: Entity | null=null,bestScore=Infinity;for(const e of this.entities.values()){if(e.id===this.selfId||e.hpPct<=0)continue;if(e.kind==='player'&&(this.zoneSafe||this.zoneOf(e.x,e.y).safe))continue;const d=Math.hypot(e.x-this.selfX,e.y-this.selfY);if(d>=range)continue;const score=d+(e.kind==='player'?90:0);if(score<bestScore){best=e;bestScore=score;}}return best;}
